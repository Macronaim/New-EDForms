...
?>
	...
	function DeleteCustomChoice() {
		if (!confirm(<?php echo json_encode( __( "Delete this custom choice list? 'OK' to delete, 'Cancel' to abort.", 'edforms' ) ); ?>))
			return;
		...
		InitBulkCustomPanel();
		LoadCustomChoices();
		DisplayCustomMessage(<?php echo json_encode( esc_html__( 'Item has been deleted.', 'edforms' ) )?>);
	}
	...
	function SaveCustomChoices() {
		...
		if (name.length == 0) {
			alert(<?php echo json_encode( esc_html__( 'Please enter name.', 'edforms' ) ); ?>);
			return;
		}
		else if (gform_custom_choices[name] && name != gform_selected_custom_choice) {
			alert(<?php echo json_encode( esc_html__( 'This custom choice name is already in use. Please enter another name.', 'edforms' ) ); ?>);
			return;
		}
		...
		DisplayCustomMessage(<?php echo json_encode( esc_html__( 'Item has been saved.', 'edforms' ) ); ?>);
	}
	...
	function InitializeFormConditionalLogic() {
		...
		if (canHaveConditionalLogic) {
			...
			jQuery("#form_button_conditional_logic_container").show().html("<span class='instruction' style='margin-left:0'>" + <?php echo json_encode( esc_html__( 'To use conditional logic, please create a field that supports conditional logic.', 'edforms' ) ); ?> + "</span>");
		}
	}
	...
	function InitPaginationOptions(isInit) {
		...
		for (var i = 0; i < pages.length; i++) {
			...
			str += "<li><label class='inline' for='gform_pagename_" + i + "' >" + <?php echo json_encode( esc_html__( 'Page', 'edforms' ) ); ?> + " " + (i + 1) + "</label> <input type='text' class='fieldwidth-4' id='gform_pagename_" + i + "' value='" + pageName + "' /></li>";
		}
		str += "</ul>";
		...
		if (jQuery("#pagination_type_none").is(":checked")) {
			...
			var completion_text = form["pagination"] && form["pagination"]["display_progressbar_on_confirmation"] ? form["pagination"]["progressbar_completion_text"] : <?php echo json_encode( esc_html__( 'Completed','edforms' ) ); ?>;
	...
	function TogglePostCategoryInitialItem(isInit) {
		...
		if (jQuery("#gfield_post_category_initial_item_enabled").is(":checked")) {
			...
			if (!isInit) {
				jQuery("#field_post_category_initial_item").val(<?php echo json_encode( esc_html__( 'Select a category', 'edforms' ) ); ?>);
			}
		}
		else {
	...
	function CreateInputNames(field) {
		...
		if (!inputs || GetInputType(field) == "checkbox") {
			field_str = "<label for='field_input_name' class='inline'>" + <?php echo json_encode( esc_html__( 'Parameter Name:', 'edforms' ) ); ?> + "&nbsp;</label>";
			field_str += "<input type='text' value='" + field["inputName"] + "' id='field_input_name' />";
		}
		else {
			var priceId = field['id'] + 0.2;
			field_str = "<table><tr><td><strong>Field</strong></td><td><strong>" + <?php echo json_encode( esc_html__( 'Parameter Name', 'edforms' ) ); ?> + "</strong></td></tr>";
	...
	function CreateDefaultValuesUI(field) {
		...
		if (!field['inputs']) {
			field_str = "<label for='field_single_default_value' class='inline'>" + <?php echo json_encode( esc_html__( 'Default Value:', 'edforms' ) ); ?> + "&nbsp;</label>";
			...
			field_str = "<table class='default_input_values'><tr><td><strong>Field</strong></td><td><strong>" + <?php echo json_encode( esc_html__( 'Default Value', 'edforms' ) ); ?> + "</strong></td></tr>";
			for (var i = 0; i < field["inputs"].length; i++) {
				id = field["inputs"][i]["id"];
	...
	function CreatePlaceholdersUI(field) {
		...
		if (!field["inputs"]) {
			field_str = "<label for='field_single_placeholder' class='inline'>" + <?php echo json_encode( esc_html__( 'Placeholder:', 'edforms' ) ); ?> + "&nbsp;</label>";
			...
			field_str = "<table class='input_placeholders'><tr><td><strong>Field</strong></td><td><strong>" + <?php echo json_encode( esc_html__( 'Placeholder', 'edforms' ) ); ?> + "</strong></td></tr>";
			for (var i = 0; i < field["inputs"].length; i++) {
				id = field["inputs"][i]["id"];
				inputName = 'input_' + id.toString();
				inputId = inputName.replace('.', '_');
	...
	function GetCustomizeInputsUI(field, showInputSwitches) {
		...
		if (!field['inputs']) {
			html = "<label for='field_single_input' class='inline'>" + <?php echo json_encode( esc_html__( 'Sub-Label:', 'edforms' ) ); ?> + "&nbsp;</label>";
			...
			if (showInputSwitches) {
				html += "<td><strong>" + <?php echo json_encode( esc_html__( 'Show', 'edforms' ) ); ?>+ "</strong></td>";
			}
			html += "<td><strong><?php esc_html_e( 'Field', 'edforms' );?></strong></td><td><strong>" + <?php echo json_encode( esc_html__( 'Custom Sub-Label', 'edforms' ) ); ?> + "</strong></td></tr>";
			for (var i = 0; i < field["inputs"].length; i++) {
				...
				title = isHidden ? <?php echo json_encode( esc_html__( 'Inactive', 'edforms' ) ); ?> : <?php echo json_encode( esc_html__( 'Active', 'edforms' ) ); ?>;
	...
	function SetCopyValuesOptionProperties(isEnabled) {
		var defaultLabel = <?php echo json_encode( esc_html__( 'Same as previous', 'edforms' ) ); ?>;
	...
	function ToggleInputHidden(img, inputId) {
		...
		if (isHidden) {
			...
			jQuery(img).attr('title', <?php echo json_encode( esc_html__( 'Active', 'edforms' ) ); ?>).attr('alt', <?php echo json_encode( esc_html__( 'Active', 'edforms' ) ); ?>);
		}
		else {
			img.src = img.src.replace("active1.png", "active0.png");
			jQuery(img).attr('title', <?php echo json_encode( esc_html__( 'Inactive', 'edforms' ) ); ?>).attr('alt', <?php echo json_encode( esc_html__( 'Inactive', 'edforms' ) ); ?>);
		}
	...
	function SetProductField(field) {
		...
		if (productFields.length < 1) {
			jQuery("#product_field").hide().after("<div id='gform_no_product_field_message'>" + <?php echo json_encode( esc_html__( 'This field is not associated with a product. Please add a Product Field to the form.', 'edforms' ) ); ?> + "</div>");
		}
		else {
			...
			if (!is_selected && field["productField"] != "") {
				product_field.append("<option value='" + field["productField"] + "' selected='selected'>[" + <?php echo json_encode( esc_html__( 'Deleted Field', 'edforms' ) ); ?> + "]</option>");
			}
	...
	function LoadFieldConditionalLogic(isEnabled, objectType) {
		...
		if (isEnabled) {
			...
			jQuery("#" + objectType + "_conditional_logic_container").show().html("<span class='instruction' style='margin-left:0'>" + <?php echo json_encode( esc_html__( 'To use conditional logic, please create a field that supports conditional logic.', 'edforms' ) ); ?> + "</span>");
		}
	}
	...
	function ToggleColumns(isInit) {
		...
		if (jQuery('#field_columns_enabled').is(":checked")) {
			...
			if (!field.choices)
				field.choices = new Array(new Choice(<?php echo json_encode( esc_html__( 'Column 1', 'edforms' ) ); ?>), new Choice(<?php echo json_encode( esc_html__( 'Column 2', 'edforms' ) ); ?>), new Choice(<?php echo json_encode( esc_html__( 'Column 3', 'edforms' ) ); ?>));
	...
	function DuplicateTitleMessage() {
		...
		alert(<?php echo json_encode( esc_html__( 'The form title you have entered is already taken. Please enter a unique form title', 'edforms' ) ); ?>);
	}

	function ValidateForm() {
		var error = "";
		if (jQuery.trim(form.title).length == 0) {
			error = <?php echo json_encode( esc_html__( 'Please enter a Title for this form. When adding the form to a page or post, you will have the option to hide the title.', 'edforms' ) ); ?>;
		}
		else {
				...
				switch (field["type"]) {
					case "page" :
						if (i == last_page_break + 1 || i == form["fields"].length - 1)
							error = <?php echo json_encode( esc_html__( 'Your form currently has one or more pages without any fields in it. Blank pages are a result of Page Breaks that are positioned as the first or last field in the form or right after each other. Please adjust your Page Breaks and try again.', 'edforms' ) ); ?>;
	...
	function SetDefaultValues( field, index ) {
		...
		switch (inputType) {
			...
			case "post_category" :
				field.label = <?php echo json_encode( esc_html__( 'Post Category', 'edforms' ) ); ?>;
				field.inputs = null;
			...
			case "section" :
				field.label = <?php echo json_encode( esc_html__( 'Section Break', 'edforms' ) ); ?>;
				field.inputs = null;
			...
			case "page" :
				...
				field["nextButton"]["text"] = <?php echo json_encode( esc_html__( 'Next', 'edforms' ) ); ?>;
				field["previousButton"] = new Button();
				field["previousButton"]["text"] = <?php echo json_encode( esc_html__( 'Previous', 'edforms' ) ); ?>;
				break;

			case "html" :
				field.label = <?php echo json_encode( esc_html__( 'HTML Block', 'edforms' ) ); ?>;
				field.inputs = null;
				field["displayOnly"] = true;
				break;
			...
			case "list" :
				if (!field.label)
					field.label = <?php echo json_encode( esc_html__( 'List', 'edforms' ) ); ?>;
			...
			case "name" :
				if (!field.label){
					field.label = <?php echo json_encode( esc_html__( 'Name', 'edforms' ) ); ?>;
				}
			...
			case "checkbox" :
				if (!field.label)
					field.label = <?php echo json_encode( esc_html__( 'Untitled', 'edforms' ) ); ?>;

				if (!field.choices)
					field.choices = new Array(new Choice(<?php echo json_encode( esc_html__( 'First Choice', 'edforms' ) ); ?>), new Choice(<?php echo json_encode( esc_html__( 'Second Choice', 'edforms' ) ); ?>), new Choice(<?php echo json_encode( esc_html__( 'Third Choice', 'edforms' ) ); ?>));
			...
			case "radio" :
				if (!field.label)
					field.label = "<?php _e( 'Untitled', 'edforms' ); ?>";

				field.inputs = null;
				if (!field.choices) {
					field.choices = field["enablePrice"] ? new Array(new Choice(<?php echo json_encode( esc_html__( 'First Choice', 'edforms' ) ); ?>, "", "0.00"), new Choice(<?php echo json_encode( esc_html__( 'Second Choice', 'edforms' ) ); ?>, "", "0.00"), new Choice(<?php echo json_encode( esc_html__( 'Third Choice', 'edforms' ) ); ?>, "", "0.00"))
						: new Array(new Choice(<?php echo json_encode( esc_html__( 'First Choice', 'edforms' ) ); ?>), new Choice(<?php echo json_encode( esc_html__( 'Second Choice', 'edforms' ) ); ?>), new Choice(<?php echo json_encode( esc_html__( 'Third Choice', 'edforms' ) ); ?>));
				}
				break;
			...
			case "address" :
				...
				if (!field.label)
					field.label = <?php echo json_encode( esc_html__( 'Address', 'edforms' ) ); ?>;
				field.addressType = <?php echo json_encode( GF_Fields::get( 'address' )->get_default_address_type( rgget( 'id' ) ) ) ?>;
				field.inputs = [new Input(field.id + 0.1, <?php echo json_encode( gf_apply_filters( array( 'gform_address_street', rgget( 'id' ) ), esc_html__( 'Street Address', 'edforms' ), rgget( 'id' ) ) ); ?>), new Input(field.id + 0.2, <?php echo json_encode( gf_apply_filters( array( 'gform_address_street2', rgget( 'id' ) ), esc_html__( 'Address Line 2', 'edforms' ), rgget( 'id' ) ) ); ?>), new Input(field.id + 0.3, <?php echo json_encode( gf_apply_filters( array( 'gform_address_city', rgget( 'id' ) ), esc_html__( 'City', 'edforms' ), rgget( 'id' ) ) ); ?>),
					new Input(field.id + 0.4, <?php echo json_encode( gf_apply_filters( array( 'gform_address_state', rgget( 'id' ) ), __( 'State / Province', 'edforms' ), rgget( 'id' ) ) ); ?>), new Input(field.id + 0.5, <?php echo json_encode( gf_apply_filters( array( 'gform_address_zip', rgget( 'id' ) ), esc_html__( 'ZIP / Postal Code', 'edforms' ), rgget( 'id' ) ) ); ?>), new Input(field.id + 0.6, <?php echo json_encode( gf_apply_filters( array( 'gform_address_country', rgget( 'id' ) ), esc_html__( 'Country', 'edforms' ), rgget( 'id' ) ) ); ?>)];
				break;
			case "creditcard" :

				if (!field.label)
					field.label = <?php echo json_encode( esc_html__( 'Credit Card', 'edforms' ) ); ?>;
				var ccNumber, ccExpirationMonth, ccExpirationYear, ccSecruityCode, ccCardType, ccName;

				ccNumber = new Input(field.id + ".1", <?php echo json_encode( gf_apply_filters( array( 'gform_card_number', rgget( 'id' ) ), esc_html__( 'Card Number', 'edforms' ), rgget( 'id' ) ) ); ?>);
				ccExpirationMonth = new Input(field.id + ".2_month", <?php echo json_encode( gf_apply_filters( array( 'gform_card_expiration', rgget( 'id' ) ), esc_html__( 'Expiration Month', 'edforms' ), rgget( 'id' ) ) ); ?>);
				ccExpirationMonth.defaultLabel = <?php echo json_encode( esc_html__( 'Expiration Date', 'edforms' ) ); ?>;
				ccExpirationYear = new Input(field.id + ".2_year", <?php echo json_encode( gf_apply_filters( array( 'gform_card_expiration', rgget( 'id' ) ), esc_html__( 'Expiration Year', 'edforms' ), rgget( 'id' ) ) ); ?>);
				ccSecruityCode = new Input(field.id + ".3", <?php echo json_encode( gf_apply_filters( array( 'gform_card_security_code', rgget( 'id' ) ), esc_html__( 'Security Code', 'edforms' ), rgget( 'id' ) ) ); ?>);
				ccCardType = new Input(field.id + ".4", <?php echo json_encode( gf_apply_filters( array( 'gform_card_type', rgget( 'id' ) ), __( 'Card Type', 'edforms' ), rgget( 'id' ) ) ); ?>);
				ccName = new Input(field.id + ".5", <?php echo json_encode( gf_apply_filters( array( 'gform_card_name', rgget( 'id' ) ), esc_html__( 'Cardholder Name', 'edforms' ), rgget( 'id' ) ) ); ?>);
			...
			case "email" :
				...
				if (!field.label)
					field.label = <?php echo json_encode( esc_html__( 'Email', 'edforms' ) ); ?>;
			...
			case "number" :
				...
				if (!field.label)
					field.label = <?php echo json_encode( esc_html__( 'Number', 'edforms' ) ); ?>;
			...
			case "phone" :
				field.inputs = null;
				if (!field.label)
					field.label = <?php echo json_encode( esc_html__( 'Phone', 'edforms' ) ); ?>;
			...
			case "date" :
				...
				if (!field.label)
					field.label = <?php echo json_encode( esc_html__( 'Date', 'edforms' ) ); ?>;
				break;
			case "time" :
				field.inputs = GetTimeFieldInputs(field);
				if (!field.label)
					field.label = <?php echo json_encode( esc_html__( 'Time', 'edforms' ) ); ?>;
				break;
			case "website" :
				field.inputs = null;
				if (!field.label)
					field.label = <?php echo json_encode( esc_html__( 'Website', 'edforms' ) ); ?>;
			...
			case "password" :
				...
				field["displayOnly"] = true;
				if (!field.label)
					field.label = <?php echo json_encode( esc_html__( 'Password', 'edforms' ) ); ?>;
				break;
			case "fileupload" :
				field.inputs = null;
				if (!field.label)
					field.label = <?php echo json_encode( esc_html__( 'File', 'edforms' ) ); ?>;
				break;
			case "hidden" :
				field.inputs = null;
				if (!field.label)
					field.label = <?php echo json_encode( esc_html__( 'Hidden Field', 'edforms' ) ); ?>;
				break;
			case "post_title" :
				field.inputs = null;
				field.label = <?php echo json_encode( esc_html__( 'Post Title', 'edforms' ) ); ?>;
				break;
			case "post_content" :
				field.inputs = null;
				field.label = <?php echo json_encode( esc_html__( 'Post Body', 'edforms' ) ); ?>;
				break;
			case "post_excerpt" :
				field.inputs = null;
				field.label = <?php echo json_encode( esc_html__( 'Post Excerpt', 'edforms' ) ); ?>;
				field.size = "small";
				break;
			case "post_tags" :
				field.inputs = null;
				field.label = <?php echo json_encode( esc_html__( 'Post Tags', 'edforms' ) ); ?>;
				field.size = "large";
				break;
			case "post_custom_field" :
				...
				field.label = <?php echo json_encode( esc_html__( 'Post Custom Field', 'edforms' ) ); ?>;
				break;
			case "post_image" :
				field.label = <?php echo json_encode( esc_html__( 'Post Image', 'edforms' ) ); ?>;
				field.inputs = null;
			...
			case "captcha" :
				...
				field.label = <?php echo json_encode( esc_html__( 'CAPTCHA', 'edforms' ) ); ?>;
			...
			case "product" :
			case "hiddenproduct" :
				field.label = <?php echo json_encode( esc_html__( 'Product Name', 'edforms' ) ); ?>;
				field.inputs = null;
				...
				if (field.inputType == "singleproduct" || field.inputType == "hiddenproduct" || field.inputType == "calculation") {
					...
					field.inputs = [new Input(field_id + 0.1, <?php echo json_encode( esc_html__( 'Name', 'edforms' ) ); ?>), new Input(field_id + 0.2, <?php echo json_encode( esc_html__( 'Price', 'edforms' ) ); ?>), new Input(field_id + 0.3, <?php echo json_encode( esc_html__( 'Quantity', 'edforms' ) ); ?>)];
			...
			case "singleshipping" :
			case "shipping" :
				field.label = <?php echo json_encode( esc_html__( 'Shipping', 'edforms' ) ); ?>;
				field.inputs = null;
				...
				break;
			case "total" :
				field.label = <?php echo json_encode( esc_html__( 'Total', 'edforms' ) ); ?>;
				field.inputs = null;

				break;

			case "option" :
				field.label = <?php echo json_encode( esc_html__( 'Option', 'edforms' ) ); ?>;

				if (!field.inputType)
					field.inputType = "select";

				if (!field.choices) {
					field.choices = new Array(new Choice(<?php echo json_encode( esc_html__( 'First Option', 'edforms' ) ); ?>, "", "0.00"), new Choice(<?php echo json_encode( esc_html__( 'Second Option', 'edforms' ) ); ?>, "", "0.00"), new Choice(<?php echo json_encode( esc_html__( 'Third Option', 'edforms' ) ); ?>, "", "0.00"));
				}
			...
			case "donation" :
				...
				field.label = <?php echo json_encode( esc_html__( 'Donation', 'edforms' ) ); ?>;
			...
			case "price" :
				...
				field.label = <?php echo json_encode( esc_html__( 'Price', 'edforms' ) ); ?>;
			...
			case "quantity" :
				field.label = <?php echo json_encode( esc_html__( 'Quantity', 'edforms' ) ); ?>;
	...
	function GetAdvancedNameFieldInputs(field, prefixHidden, middleHidden, suffixHidden) {
		var prefixInput = new Input(field.id + '.2', <?php echo json_encode( gf_apply_filters( array( 'gform_name_prefix', rgget( 'id' ) ), esc_html__( 'Prefix', 'edforms' ), rgget( 'id' ) ) ); ?>);
		prefixInput.choices = GetDefaultPrefixChoices();
		prefixInput.isHidden = prefixHidden;

		var firstInput = new Input(field.id + '.3', <?php echo json_encode( gf_apply_filters( array( 'gform_name_first', rgget( 'id' ) ), esc_html__( 'First', 'edforms' ), rgget( 'id' ) ) ); ?>);
		 ...
		 */
		var middleInput = new Input(field.id + '.4', <?php echo json_encode( gf_apply_filters( array( 'gform_name_middle', rgget( 'id' ) ), esc_html__( 'Middle', 'edforms' ), rgget( 'id' ) ) ); ?>);
		middleInput.isHidden = middleHidden;

		var lastInput = new Input(field.id + '.6', <?php echo json_encode( gf_apply_filters( array( 'gform_name_last', rgget( 'id' ) ), esc_html__( 'Last', 'edforms' ), rgget( 'id' ) ) ); ?>);
		var suffixInput = new Input(field.id + '.8', <?php echo json_encode( gf_apply_filters( array( 'gform_name_suffix', rgget( 'id' ) ), esc_html__( 'Suffix', 'edforms' ), rgget( 'id' ) ) ); ?>);
	...
	function GetDateFieldInputs(field) {
		...
		switch (field.dateType) {
			case 'datefield' :
				month = new Input(field.id + '.1', <?php echo json_encode( _x( 'MM', 'Abbreviation: Month', 'edforms' ) ); ?>);
				day = new Input(field.id + '.2', <?php echo json_encode( esc_html__( 'DD', 'edforms' ) ); ?>);
				year = new Input(field.id + '.3', <?php echo json_encode( esc_html__( 'YYYY', 'edforms' ) ); ?>);
				break;
			case 'datedropdown' :
				month = new Input(field.id + '.1', <?php echo json_encode( esc_html__( 'Month', 'edforms' ) ); ?>);
				month.placeholder = <?php echo json_encode( esc_html__( 'Month', 'edforms' ) ); ?>;
				day = new Input(field.id + '.2', <?php echo json_encode( esc_html__( 'Day', 'edforms' ) );?>);
				day.placeholder = <?php echo json_encode( esc_html__( 'Day', 'edforms' ) ); ?>;
				year = new Input(field.id + '.3', <?php echo json_encode( esc_html__( 'Year', 'edforms' ) ); ?>);
				year.placeholder = <?php echo json_encode( esc_html__( 'Year', 'edforms' ) ); ?>;
				break;
			default:
		}
	...
	function GetTimeFieldInputs(field) {
		...
		hour = new Input(field.id + '.1', <?php echo json_encode( esc_html__( 'HH', 'edforms' ) )?>);
		min = new Input(field.id + '.2', <?php echo json_encode( _x( 'MM', 'Abbreviation: Minutes', 'edforms' ) )?>);
		ampm = new Input(field.id + '.3', <?php echo json_encode( esc_html__( 'AM/PM', 'edforms' ) )?>);
	...
	function GetEmailFieldInputs(field) {
		...
		email = new Input(field.id, <?php echo json_encode( esc_html__( 'Enter Email', 'edforms' ) ); ?>);
		confirmation = new Input(field.id + '.2', <?php echo json_encode( esc_html__( 'Confirm Email', 'edforms' ) ); ?>);
	...
	function GetPasswordFieldInputs(field) {
		...
		password = new Input(field.id, <?php echo json_encode( esc_html__( 'Enter Password', 'edforms' ) ); ?>);
		confirmation = new Input(field.id + '.2', <?php echo json_encode( esc_html__( 'Confirm Password', 'edforms' ) ); ?>);
	...
	function UpgradeCreditCardField(field) {
		...
		if (legacyExpirationInput) {
			var monthInput = new Input(field.id + ".2_month", <?php echo json_encode( gf_apply_filters( array( 'gform_card_expiration', rgget( 'id' ) ), esc_html__( 'Expiration Month', 'edforms' ), rgget( 'id' ) ) ); ?>);
			monthInput.defaultLabel = <?php echo json_encode( esc_html__( 'Expiration Date', 'edforms' ) ); ?>;
			var yearInput = new Input(field.id + ".2_year", <?php echo json_encode( esc_html__( 'Expiration Year', 'edforms' ) ); ?>);
			field.inputs.splice(1, 1, monthInput, yearInput);
			var nameInput = GetInput(field, field.id + ".5");
			nameInput.label = <?php echo json_encode( gf_apply_filters( array( 'gform_card_name', rgget( 'id' ) ), __( 'Cardholder Name', 'edforms' ), rgget( 'id' ) ) ); ?>;
		}
	...
	function GetDefaultPrefixChoices() {
		return new Array(new Choice(<?php echo json_encode( esc_html__( 'Mr.', 'edforms' ) ); ?>), new Choice(<?php echo json_encode( esc_html__( 'Mrs.', 'edforms' ) ); ?>), new Choice(<?php echo json_encode( esc_html__( 'Miss', 'edforms' ) ); ?>), new Choice(<?php echo json_encode( esc_html__( 'Ms.', 'edforms' ) ); ?>), new Choice(<?php echo json_encode( esc_html__( 'Dr.', 'edforms' ) ); ?>), new Choice(<?php echo json_encode( esc_html__( 'Prof.', 'edforms' ) ); ?>), new Choice(<?php echo json_encode( esc_html__( 'Rev.', 'edforms' ) ); ?>));
	}
		...
		switch (type) {
			case "captcha" :
				if (GetFieldsByType(["captcha"]).length > 0) {
					alert(<?php echo json_encode( esc_html__( 'Only one reCAPTCHA field can be added to the form', 'edforms' ) ); ?>);
					return false;
				}
				break;
			...
			case "shipping" :
				if (GetFieldsByType(["shipping"]).length > 0) {
					alert(<?php echo json_encode( esc_html__( 'Only one Shipping field can be added to the form', 'edforms' ) ); ?>);
					return false;
				}
				break;
			...
			case "post_content" :
				if (GetFieldsByType(["post_content"]).length > 0) {
					alert(<?php echo json_encode( esc_html__( 'Only one Post Content field can be added to the form', 'edforms' ) ); ?>);
					return false;
				}
				break;
			case "post_title" :
				if (GetFieldsByType(["post_title"]).length > 0) {
					alert(<?php echo json_encode( esc_html__( 'Only one Post Title field can be added to the form', 'edforms' ) ); ?>);
					return false;
				}
				break;
			case "post_excerpt" :
				if (GetFieldsByType(["post_excerpt"]).length > 0) {
					alert(<?php echo json_encode( esc_html__( 'Only one Post Excerpt field can be added to the form', 'edforms' ) ); ?>);
					return false;
				}
				break;
			case "creditcard" :
				if (GetFieldsByType(["creditcard"]).length > 0) {
					alert(<?php echo json_encode( esc_html__( 'Only one credit card field can be added to the form', 'edforms' ) ); ?>);
					return false;
				}
				break;
			case "quantity" :
			case "option" :
				if (GetFieldsByType(["product"]).length <= 0) {
					alert(<?php echo json_encode( esc_html__( 'You must add a product field to the form first', 'edforms' ) ); ?>);
					return false;
				}
				break;
			default :
	...
	function StartAddField(type, index) {
		...
		mysack.onError = function () {
			alert(<?php echo json_encode( esc_html__( 'Ajax error while adding field', 'edforms' ) ); ?>)
		};
		mysack.runAJAX();
	...
	function StartChangeInputType(type, field) {
		...
		mysack.setVar('form_id', form.id);
		mysack.onError = function () {
			alert(<?php echo json_encode( esc_html__( 'Ajax error while changing input type', 'edforms' ) ); ?>)
		};
		mysack.runAJAX();
	...
	function SetFieldPhoneFormat(phoneFormat) {
		var instruction = phoneFormat == "standard" ? <?php echo json_encode( esc_html__( 'Phone format:', 'edforms' ) ); ?> + " (###) ###-####" : "";
	...
	function LoadMessageVariables() {
		var options = "<option>" + <?php echo json_encode( esc_html__( 'Select a field', 'edforms' ) ); ?> + "</option><option value='{form_title}'>" + <?php echo json_encode( esc_html__( 'Form Title', 'edforms' ) ); ?> + "</option><option value='{date_mdy}'>" + <?php echo json_encode( esc_html__( 'Date', 'edforms' ) ); ?> + " (mm/dd/yyyy)</option><option value='{date_dmy}'>" + <?php echo json_encode( esc_html__( 'Date', 'edforms' ) ); ?> + " (dd/mm/yyyy)</option><option value='{ip}'>" + <?php echo json_encode( esc_html__( 'User IP Address', 'edforms' ) ); ?> + "</option><option value='{all_fields}'>" + <?php echo json_encode( esc_html__( 'All Submitted Fields', 'edforms' ) ); ?> + "</option>";
