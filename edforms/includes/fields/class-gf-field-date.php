...
class GF_Field_Date extends GF_Field {
	...
	public function get_form_editor_field_title() {
		return esc_attr__( 'Date', 'edforms' );
	}
	...
	public function validate( $value, $form ) {
		...
		if ( ! empty( $value ) ) {
			...
			if ( empty( $date ) || ! $this->checkdate( $date['month'], $date['day'], $date['year'] ) ) {
				...
				$message                  = $this->dateType == 'datepicker' ? sprintf( esc_html__( 'Please enter a valid date in the format (%s).', 'edforms' ), $format_name ) : esc_html__( 'Please enter a valid date.', 'edforms' );
	...
	public function get_field_input( $form, $value = '', $entry = null ) {
		...
		$month_sub_label = rgar( $month_input, 'customLabel' ) != '' ? $month_input['customLabel'] : esc_html( _x( 'MM', 'Abbreviation: Month', 'edforms' ) );
		$day_sub_label   = rgar( $day_input, 'customLabel' ) != '' ? $day_input['customLabel'] : esc_html__( 'DD', 'edforms' );
		$year_sub_label  = rgar( $year_input, 'customLabel' ) != '' ? $year_input['customLabel'] : esc_html__( 'YYYY', 'edforms' );
	...
	private function get_day_dropdown( $name = '', $id = '', $selected_value = '', $tabindex = '', $disabled_text = '', $placeholder = '' ) {
		if ( $placeholder == '' ) {
			$placeholder = esc_html__( 'Day', 'edforms' );
		}
