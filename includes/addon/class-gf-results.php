...
if ( ! class_exists( 'GFResults' ) ) {
	...
	class GFResults {
		...
		public function __construct( $slug, $config ) {
			...
			$this->_search_title = rgempty( 'search_title', $config ) ? esc_html__( 'Results Filters', 'edforms' ) : rgar( $config, 'search_title' );
		...
		public static function localize_results_scripts() {
			...
			$strings = array(
				'ajaxError' => esc_html__( 'Error retrieving results. If the problem persists, please contact support.', 'edforms' )
			);
		...
		public function filter_menu_items( $menu_items, $form_id, $compact ) {
			...
			if ( false === empty( $results_fields ) ) {
				...
				$sub_menu_items[] = array(
					'label'        => $this->_title,
					'icon'         => $icon,
					'title'        => esc_html__( 'View results generated by this form', 'edforms' ),
				...
				if ( isset( $menu_items['results'] ) ) {
					...
					$menu_items['results']['label']          = esc_html__( 'Results', 'edforms' );
					...
					$menu_items['results'] = array(
						'label'          => $compact ? esc_html__( 'Results', 'edforms' ) : $this->_title,
						'icon'           => $icon,
						'title'          => esc_attr__( 'View results generated by this form', 'edforms' ),
						'url'            => '',
			...
			<div class="wrap gforms_edit_form <?php echo GFCommon::get_browser_class() ?>">
				...
				<?php if ( false === empty( $all_fields ) ) : ?>
							...
							<div id="gresults-results-filter" class="postbox">
								...
								<div id="gresults-results-filter-content">
									<form id="gresults-results-filter-form" action="" method="GET">
										...
										$filter_ui = array(
											'fields'     => array(
												'label'   => esc_attr__( 'Filters', 'edforms' ),
											...
											'date_range' => array(
												'label'   => esc_attr__( 'Date Range', 'edforms' ),
												'tooltip' => 'gresults_date_range',
												'markup'  => '<div style="width:90px; float:left; ">
																	<label
																		for="gresults-results-filter-date-start">' .  esc_html__( 'Start', 'edforms' ) . '</label>
																...
																<div style="width:90px; float:left; ">
																	<label
																		for="gresults-results-filter-date-end">' . esc_html__( 'End', 'edforms' ) . '</label>
										...
										<div id="gresults-results-filter-buttons">
											<input type="submit" id="gresults-results-filter-submit-button"
											       ...
											       value="<?php esc_attr_e( 'Apply filters', 'edforms' ); ?>">
											<input type="button" id="gresults-results-filter-clear-button"
											       class="button button-secondary button-large"
											       value="<?php esc_attr_e( 'Clear', 'edforms' ); ?>"
											...
											<div class="gresults-filter-loading"
												...
												<i class='gficon-edforms-spinner-icon gficon-spin'></i>
											</div>
										</div>
									</form>
								</div>
					...
					<div class="gresults-filter-loading" style="display:none;margin:0 5px 10px 0;">
						<i class='gficon-edforms-spinner-icon gficon-spin'></i>&nbsp;
						<a href="javascript:void(0);" onclick="javascript:gresultsAjaxRequest.abort()" onkeypress="javascript:gresultsAjaxRequest.abort()"><?php esc_html_e( 'Cancel', 'edforms' ); ?></a>
					</div>
				...
				<?php
					...
					_e( 'This form does not have any fields that can be used for results', 'edforms' );
				endif ?>
			</div>
		...
		public static function add_tooltips( $tooltips ) {
			$tooltips['gresults_total_score']          = '<h6>' . esc_html__( 'Total Score', 'edforms' ) . '</h6>' . esc_html__( 'Scores are weighted calculations. Items ranked higher are given a greater score than items that are ranked lower. The total score for each item is the sum of the weighted scores.', 'edforms' );
			$tooltips['gresults_agg_rank']             = '<h6>' . esc_html__( 'Aggregate Rank', 'edforms' ) . '</h6>' . esc_html__( 'The aggregate rank is the overall rank for all entries based on the weighted scores for each item.', 'edforms' );
			$tooltips['gresults_date_range']           = '<h6>' . esc_html__( 'Date Range', 'edforms' ) . '</h6>' . esc_html__( 'Date Range is optional, if no date range is specified it will be ignored.', 'edforms' );
			$tooltips['gresults_filters']              = '<h6>' . esc_html__( 'Filters', 'edforms' ) . '</h6>' . esc_html__( 'Narrow the results by adding filters. Note that some field types support more options than others.', 'edforms' );
			$tooltips['gresults_average_row_score']    = '<h6>' . esc_html__( 'Average Row Score', 'edforms' ) . '</h6>' . esc_html__( 'The average (mean) score for each row: the sum of all the scores for each row divided by the total number of entries.', 'edforms' );
			$tooltips['gresults_average_global_score'] = '<h6>' . esc_html__( 'Average Global Score', 'edforms' ) . '</h6>' . esc_html__( 'The average (mean) score for the whole field. The sum of the total scores divided by the number of entries.', 'edforms' );
			$tooltips['gresults_average_score']        = '<h6>' . esc_html__( 'Average Score', 'edforms' ) . '</h6>' . esc_html__( 'The average (mean) score: The sum of the scores divided by the number of entries.', 'edforms' );
		...
		public function ajax_get_results() {
			...
			if ( $total_entries == 0 ) {
				$html = esc_html__( 'No results.', 'edforms' );
			} else {
				...
				if ( isset( $_POST['state'] ) ) {
					...
					if ( $generated_check_sum !== $posted_check_sum ) {
						...
						$output['html']   = esc_html__( 'There was an error while processing the entries. Please contact support.', 'edforms' );
				...
				if ( 'incomplete' === rgar( $data, 'status' ) ) {
					...
					$output['html']        = sprintf( esc_html__( 'Entries processed: %1$d of %2$d', 'edforms' ), rgar( $data, 'offset' ), $entry_count );
					echo json_encode( $output );
					die();
				}
		...
		public static function get_field_results( $form_id, $data, $field, $search_criteria ) {
			...
			if ( empty( $data['entry_count'] ) || empty ( $data['field_data'] ) ) {
				return esc_html__( 'No entries for this field', 'edforms' );
			}
			...
			switch ( $field_type ) {
				...
				case 'multiselect' :
					...
					if ( empty( $non_zero_results ) ) {
						$field_results .= esc_html__( 'No entries for this field', 'edforms' );
					...
					$data_table    = array();
					$data_table [] = array( esc_html__( 'Choice', 'edforms' ), esc_html__( 'Frequency', 'edforms' ) );
					...
					$chart_options = array(
						...
						'hAxis'     => array(
							...
							'title'          => esc_html__( 'Frequency', 'edforms' ),
						)
				...
				case 'likert' :
					...
					if ( $multiple_rows && $scoring_enabled ) {
						$field_results .= sprintf( '<td>%s %s</td>', esc_html__( 'Average Score', 'edforms' ), gform_tooltip( 'gresults_average_row_score', null, true ) );
					}
					...
					if ( $scoring_enabled ) {
						...
						if ( $multiple_rows ) {
							...
							$field_results .= sprintf( "<div class='gsurvey-likert-score'>%s %s: %s</div>", esc_html__( 'Average global score', 'edforms' ), $average_global_score_tooltip, $average_score );
						} else {
							$field_results .= sprintf( "<div class='gsurvey-likert-score'>%s %s: %s</div>", esc_html__( 'Average score', 'edforms' ), gform_tooltip( 'gresults_average_score', null, true ), $average_score );
						}
					}
				...
				case 'rank' :
					...
					$field_results .= esc_html__( 'Item', 'edforms' );
					$field_results .= '         </td>';
					$field_results .= "         <td class='gresults-rank-field-score'>";
					$field_results .= esc_html__( 'Total Score', 'edforms' ) . '&nbsp;' . gform_tooltip( 'gresults_total_score', null, true );
					$field_results .= '         </td>';
					$field_results .= "         <td class='gresults-rank-field-rank'>";
					$field_results .= esc_html__( 'Aggregate Rank', 'edforms' ) . '&nbsp;' . gform_tooltip( 'gresults_agg_rank', null, true );
					$field_results .= '         </td>';
					$field_results .= '     </tr>';
				...
				default :
					...
					$field_results .= "<div class='gresults-results-field-sub-label'>" . esc_html__( 'Latest values:', 'edforms' ) . '</div>';
					...
					if ( $more_remaining ) {
						$field_results .= "<a id='gresults-results-field-more-link-{$field_id}' class='gresults-results-field-more-link' href='javascript:void(0)' onclick='gresults.getMoreResults({$form_id},{$field_id})' onkeypress='gresults.getMoreResults({$form_id},{$field_id})'>" . esc_html__( 'Show more', 'edforms' ) . '</a>';
					}
					break;
			}
