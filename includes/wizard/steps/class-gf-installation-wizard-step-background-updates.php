...
class GF_Installation_Wizard_Step_Background_Updates extends GF_Installation_Wizard_Step {
	...
	function display() {
		...
		<p>
			<?php
			esc_html_e( 'Ed Forms will download important bug fixes, security enhancements and plugin updates automatically. Updates are extremely important to the security of your WordPress site.', 'edforms' );
			?>
		</p>
		<p>
			<strong>
				<?php
				esc_html_e( 'This feature is activated by default unless you opt to disable it below. We only recommend disabling background updates if you intend on managing updates manually.', 'edforms' );
				?>
			</strong>
		</p>
		<?php
		...
		if ( ! $is_valid_license_key ) :
			...
			<p>
				<strong>
					<?php esc_html_e( 'Updates will only be available if you have entered a valid License Key', 'edforms' ); ?>
				</strong>
			</p>
		<?php
		endif;
		...
		<div>
			<label>
				...
				<?php esc_html_e( 'Keep background updates enabled', 'edforms' ); ?>
			</label>
		</div>
		<div>
			<label>
				...
				<?php esc_html_e( 'Turn off background updates', 'edforms' ); ?>
			</label>
		</div>
		<div id="accept_terms_container" style="display:none;">
			<div id="are_you_sure" style="background: #fff none repeat scroll 0 0;box-shadow: 0 1px 1px 0 rgba(0, 0, 0, 0.1);padding: 1px 12px;border-left: 4px solid #dd3d36;margin: 5px 0 15px;display: inline-block;">
				...
				<h3 style="margin-top:0.6em;"><i class="fa fa-exclamation-triangle gf_invalid"></i> <?php _e( 'Are you sure?', 'edforms' ); ?>
				</h3>
				<p>
					<strong><?php esc_html_e( 'By disabling background updates your site may not get critical bug fixes and security enhancements. We only recommend doing this if you are experienced at managing a WordPress site and accept the risks involved in manually keeping your WordPress site updated.', 'edforms' ); ?></strong>
				</p>
			</div>
			<label>
				<input type="checkbox" id="accept_terms" value="1" <?php checked( 1, $this->accept_terms ); ?> name="accept_terms"/>
				<?php esc_html_e( 'I understand and accept the risk of not enabling background updates.', 'edforms' ); ?> <span class="gfield_required">*</span>
			</label>
	...
	function get_title(){
		return esc_html__( 'Background Updates', 'edforms' );
	}
	...
	function summary( $echo = true ){
		$html = $this->background_updates !== 'disabled' ? esc_html__( 'Enabled', 'edforms' ) . '&nbsp;<i class="fa fa-check gf_valid"></i>' :   esc_html__( 'Disabled', 'edforms' ) . '&nbsp;<i class="fa fa-times gf_invalid"></i>' ;
		if ( $echo ) {
			echo $html;
		}
		return $html;
